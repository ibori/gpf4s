...
$g4['title'] = "플러그인 필터 설정 - " . $info->label;
...
?>
<form name="frmfltboard" action="<?=G4_ADMIN_URL?>/gpf/exe/action_filter.php" method="post" onsubmit="return check_form(this);">
  ...
  $.fn.manage_user = function(opts) {
	  return this.each(function() {
		...
		var search_user = function() {
			$.getJSON('<?=G4_ADMIN_URL?>/gpf/exe/g.php?md=user_search&mb_info='+$us.val(), function(json) {
				if(json.code == 1) {
...
function add_user(users, $slt)
{
	...
	{
		$.getJSON('<?=G4_ADMIN_URL?>/gpf/exe/g.php?md=user_info&mb_info='+users[i], function(json) {
			if(json.code == 1) {
...
function add_board(bo_tables, $slt)
{
	...
	{
		$.getJSON('<?=G4_ADMIN_URL?>/gpf/exe/g.php?md=board_info&btable='+bo_tables[i], function(json) {
			if(json.code == 1) {
