...
<p>
플러그인을 설치/제거, 활성화/비활성화, 설정 또는 순서를 변경합니다.
</p>

<section class="gpf_plugins cbox">
...
foreach($plugins as $id => $plugin) { 
	...
	if(!$info->shouldInstall()) {
		...
		if($plugin->use_filter) array_push($btns, "<a href='".G4_ADMIN_URL."/gpf/action_filter.php?p={$id}'>액션필터</a>");
		if($info->shouldSetup()) array_push($btns, "<a href='".G4_ADMIN_URL."/gpf/plugin_setup.php?p={$id}'>설정</a>");
...
<tr id="gpf_plugins_<?=$id?>" <?=($using_plugins[$id] ? "class=\"gpf_active_plugin\"" : "class=\"gpf_inactive_plugin\"")?>>
	<td valign="top">
		<img src="<?=GPF_ADMIN_URL?>/imgs/move.png" class="gpf_handle" />
...
{
	if(confirm('"'+name+'" 플러그인을 설치하시겠습니까?')) location.href = "<?=GPF_ADMIN_URL?>/plugin_install.php?p="+id;
}

function plugin_uninstall(id, name)
{
	if(confirm('"'+name+'" 플러그인을 삭제하시겠습니까?')) location.href = "<?=GPF_ADMIN_URL?>/plugin_uninstall.php?p="+id;
}
</script>
